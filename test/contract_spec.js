...
contract("FacultyRating", function () {
  ...
  it("isWhiteListed for student with valid address", async function(){
    ...
    let result = await FacultyRating.methods.isWhiteListed().call();
...
//testing addWhiteList function 
  it("add unlisted address to whitelist as admin", function(){
    FacultyRating.methods.addWhiteList(0x80c843af8e5ae9153538d759f7ad35e9461b5699).send({from: accounts[0]});
  });
  ...
  context("add listed address to whitelist", function(){
    ...
    it("as non-admin", async function(){
      try{
        await FacultyRating.methods.addWhiteList(0xe7f114591446e149ad12892c0ae5c0fa48adbcac).send({from: accounts[1]});
      }
      catch(error){
        let actualMessage = error.message;
        let expectedMessage = "Request denied, you are not admin!";
  ...
  context("remove listed people from the list", async function(){
    ...
    it("as admin", async function(){
      await FacultyRating.methods.removeFromWhiteList(0xe7f114591446e149ad12892c0ae5c0fa48adbcac).send({from: accounts[0]});
    });
    it("as non-admin", async function(){
      try{
        await FacultyRating.methods.removeFromWhiteList(0xe7f114591446e149ad12892c0ae5c0fa48adbcac).send({from: accounts[1]});
      }
      catch(error){
        let actualMessage = error.message;
        let expectedMessage = "Request denied, you are not admin!";
  ...
  context("sending feedback to existed teacher", async function(){
    let teacher=0xe7f114591446e149ad12892c0ae5c0fa48adbcac;
    let rate = 10;
    it("sending feedback to existed teacher as listed admin", async function(){
      await FacultyRating.methods.addWhiteList(accounts[0]).send({from: accounts[0]});
  ...
  context("sending feedback to nonexistent teacher address", async function(){
    let nonexistentTeacher = 0x5dc59f8f0d5a190068424a9006cf583e7abdd64c;
    let rate = 5;
